...
import { getCategoryFn, updateCategoryFn } from '#/lib/server/categories'
...
export const Route = createFileRoute('/_authed/categories/edit/$id')({
...
function EditCategoryComponent() {
  ...
  const form = useForm({
    defaultValues: {
      ...
      status: (categoryQuery.data?.data?.status || 'active') as 'active' | 'inactive',
    },
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          toast.error(res.message || 'Gagal mengupdate kategori', { id: toastId })
        }
      } catch (err: any) {
      ...
      <div className='max-w-md'>
        ...
        >
          ...
          <div className='flex gap-2 pt-4'>
            ...
            >
              {updateCategory.isPending ? 'Menyimpan...' : 'Simpan Perubahan'}
            </Button>
            <Button
              type="button"
              variant='outline'

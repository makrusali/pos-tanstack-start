...
import { getStockLocationFn, updateStockLocationFn } from '#/lib/server/stock-locations'
...
export const Route = createFileRoute('/_authed/stock-locations/edit/$id')({
  component: EditStockLocationComponent,
...
function EditStockLocationComponent() {
  ...
  const updateLocation = useMutation({
    mutationFn: updateStockLocationFn,
  })

  const form = useForm({
    defaultValues: {
      name: locationQuery.data?.data?.name || '',
      note: locationQuery.data?.data?.note || '',
      status: (locationQuery.data?.data?.status || 'active') as 'active' | 'inactive',
    },
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          toast.error(res.message || 'Gagal mengupdate lokasi stok', { id: toastId })
        }
      } catch (err: any) {
  ...
  if (locationQuery.isFetching) {
    return <StockLocationsTableSkeleton />
  }
  ...
  return (
    <div className='flex flex-1 flex-col gap-4 sm:gap-6'>
      <div className='flex items-center gap-4'>
        ...
        <div>
          <h2 className='text-2xl font-bold tracking-tight'>Edit Lokasi Stok</h2>
          <p className='text-muted-foreground'>
            Ubah informasi lokasi stok
          </p>
        </div>
      </div>
      ...
      <div className='max-w-md'>
        ...
        >
          ...
          <div className='flex gap-2 pt-4'>
            ...
            >
              {updateLocation.isPending ? 'Menyimpan...' : 'Simpan Perubahan'}
            </Button>
            <Button
              type="button"
              variant='outline'

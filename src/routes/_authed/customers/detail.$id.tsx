...
import { ArrowLeft, Phone, MapPin, Edit, ShoppingBag, Calendar } from 'lucide-react'
...
function CustomerDetailComponent() {
  ...
  if (!customer) {
    return <div className="text-center text-destructive">Customer tidak ditemukan</div>
  }
    ...
    <div className='flex flex-1 flex-col gap-4 sm:gap-6'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>Detail Customer</h2>
            <p className='text-muted-foreground'>
              Informasi lengkap tentang customer
            </p>
          </div>
        </div>
        <Button onClick={() => navigate({ to: `/customers/edit/$id`, params: { id: customer.id } })}>
        ...
        {customer.transactions && customer.transactions.length > 0 && (
          <Card className='md:col-span-2'>
            <CardHeader>
              ...
              <CardDescription>10 transaksi terakhir customer ini</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {customer.transactions.map((transaction: any) => (
                  <div key={transaction.id} className='flex justify-between items-center p-3 border rounded-lg'>
                    ...
                    <div className='text-right'>
                      <p className='font-semibold'>{formatCurrency(transaction.grand_total)}</p>

...
export const stockMovementColumns: ColumnDef<StockMovement>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <div>
                    <p className='font-medium'>{movement.stockProductSku?.productSku?.product?.name || '-'}</p>
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const movement = row.original
            return Number(movement.prev_quantity).toLocaleString()
        },
    },
    {
        ...
        cell: ({ row }) => {
            const movement = row.original
            return Number(movement.current_quantity).toLocaleString()
        },
    },
    {
        accessorKey: 'type',
        ...
        cell: ({ row }) => {
            ...
            return (
                <div>
                    ...
                    <p className='text-xs text-muted-foreground'>ID: {movement.reference_id.slice(0, 8)}</p>
                </div>
            )
        },
    },
